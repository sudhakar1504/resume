...
import Banner from '../../assest/green/project.gif';
...
const Project = ({setProjects}) => {
    ...
    const getValue = (type)=>{
        ...
        for (let i = 0; i < non.length; i++) {
            ...
            //     is_present = true
            // }
            if(!company_name  && !company_role && !company_aboutwork){
    ...
    const Gen=({countID})=>{
            ...
            return(
                ...
                <div className="c_name">
                    <label htmlFor="c_name">Project Name</label>
                    <input type="text" id="c_name" name = {countID} placeholder="Eg Todo list" autoComplete='off' defaultValue={structure?.[countID]?.name}/>
                </div>
                ...
                <div className="c_role">
                    <label htmlFor="c_role">Project URL</label>
                    <input type="text" id="c_role" placeholder='Eg Paste your project URL' autoComplete='off' defaultValue={structure?.[countID]?.role}/>
                </div>
                </div>
    
                <div className="S_date_and_E_date">
                    <div className="S_date">
                        <label htmlFor="Sdate">Project Start Date</label>
                        <input type="date" name="Sdate" id="Sdate" defaultValue={structure?.[countID]?.Sdate} />
                    </div>
                    <div className="E_date">
                        <label htmlFor="Edate">Project End Date</label>
                ...
                <div className="about_work">
                    <label htmlFor="aboutwork">Project description</label>
                    <textarea name="aboutwork" id="aboutwork" cols="30" rows="10" defaultValue={structure?.[countID]?.about} placeholder='EG briefy your project'></textarea>
                </div>
            </div>
            )
  ...
  return (
    ...
    <button onClick={()=> getValue("add")} className="addnew">Add New<i className="fa-solid fa-plus"></i></button> <br />
