...
import Banner  from './../../assest/blue/education.gif';
...
if(!company_name && !company_location && !company_role && !company_aboutwork){
...
const checkHandle =(e)=>{
  let Edate =  e.target.parentElement.parentElement.querySelector('#Edate');

  if(e.target.checked == true){

      Edate.disabled = true;
      Edate.style.opacity = '0.5'
  }else{
    ...
    const Gen=({countID})=>{
            ...
            return(
                ...
                <div className="c_name">
                    <label htmlFor="c_name">School / College Name</label>
                    <input type="text" id="c_name" name = {countID} autoComplete="off" placeholder="Eg Don bosco" defaultValue={structure?.[countID]?.name}/>
                </div>
                <div className="C_location_and_role">
                <div className="c_location">
                    <label htmlFor="c_location">Location</label>
                    <input type="text" id="c_location" placeholder='Eg Chennai' autoComplete="off" defaultValue={structure?.[countID]?.location} />
                </div>
                <div className="c_role">
                    <label htmlFor="c_role">Course</label>
                    <input type="text" id="c_role" autoComplete="off" placeholder='Eg Bachelor of computer application' defaultValue={structure?.[countID]?.role}/>
                </div>
                </div>
                ...
                <div className="S_date_and_E_date">
                    ...
                    <div className="present">
                        ...
                        <input type="checkbox" name="present" onClick={(e) =>checkHandle(e)} id="present" defaultChecked={structure?.[countID]?.present}/>
                    </div>
                    
                </div>
    
                <div className="about_work">
                    <label htmlFor="aboutwork">University / School board</label>
                    <input name="aboutwork" id="aboutwork" autoComplete="off" defaultValue={structure?.[countID]?.about} placeholder='EG Madras university / CBSE' />
                </div>
            </div>
            )
